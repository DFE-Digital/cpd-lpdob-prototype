...
router.post(`/v${verNum}/choose-provider-answer-npq`, function (req, res) {
  ...
  if (chooseProvider == "ambition institute"){
    ...
    res.redirect('/v${verNum}/financial-statement-NPQ-v9')
  } else {
    // Send user to ineligible page
    res.redirect('/v${verNum}/financial-statement-NPQ-v9-page-unavailable')
  }

})

// Run this code when a form is submitted to 'choose-provider-answer-ecf'
router.post('/v${verNum}/choose-provider-answer-ecf', function (req, res) {
  ...
  if (chooseProvider == "ambition institute"){
    // Send user to next page
    res.redirect('/v${verNum}/financial-statement-ECF-v3-B2')
  } else {
    // Send user to ineligible page
    res.redirect('/v${verNum}/financial-statement-ECF-v3-B2-page-unavailable')
  }

})

// Run this code when a form is submitted to 'choose-programme-answer'
router.post('/v${verNum}/choose-programme-answer', function (req, res) {
  ...
  if (chooseProgramme == "ecf payments"){
    // Send user to next page
    res.redirect('/v${verNum}/choose-provider-ecf')
  } else {
    // Send user to ineligible page
    res.redirect('/v${verNum}/choose-provider-npq')
  }

})

// Run this code when a form is submitted to 'add-adjustment-answer-1'
router.post('/v${verNum}/add-adjustment-answer-1', function (req, res) {
  ...
  if (needAnotherAdjustment == "no"){
    ...
    res.redirect('/v${verNum}/financial-statement-ECF-v3-B2-added-adjustment')
  } else {
    ...
    res.redirect('/v${verNum}/add-adjustment-type-2')
  }

})

// Run this code when a form is submitted to 'add-adjustment-answer-2'
router.post('/v${verNum}/add-adjustment-answer-2', function (req, res) {
  ...
  if (needAnotherAdjustment == "no"){
    // Send user back to statement version/update with Additional adjustment added
    res.redirect('/v${verNum}/page-unavailable-adding-more-than-one-adjustment')
  } else {
    // Send user to back in loop to add another adjustment (add-adjustment-type-2)
    // Note: adding more than one adjustement is not yet available in prototype
    res.redirect('/v${verNum}/page-unavailable-adding-more-than-one-adjustment')
...
router.post('/v${verNum}/remove-adjustment-answer', function (req, res) {
  ...
  if (removeAdjustment == "no"){
    ...
    res.redirect('/v${verNum}/financial-statement-ECF-v3-B2-0')
  }

})

// Run this code when a form is submitted to 'remove-adjustment-answer-2'
router.post('/v${verNum}/remove-adjustment-answer-2', function (req, res) {
  ...
  if (removeAdjustment == "no"){
    // Send user back to add to a list 
    res.redirect('/v${verNum}/adjustments-add-to-a-list-2')
  } else {
    // Send user back to statement
    res.redirect('/v${verNum}/financial-statement-ECF-v3-B2-added-adjustment')
  }
